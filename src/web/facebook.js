...
window.onload = async () => {
	...
	let hideHasRoutes = ['/watch/', '/marketplace/', '/gaming/']
	let navs = document.querySelectorAll('div[role="navigation"] ul li')
	navs && navs.forEach(nav => {
		let link = nav.querySelector('a')
		let haveToHide = link && hideHasRoutes.some(r => link.href && link.href.includes(r))
		if (haveToHide) nav.style.display = 'none'
	})
